...
- **Unowned mode is preserved.** With no `CODESK_OWNER_PID` in the environment, no watchdog starts and the process runs until signalled. This is what `pnpm run dev`, `pnpm start`, and the test suite rely on, and it is how a remote `codeskd` under systemd runs.
