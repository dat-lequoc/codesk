...
export const api = {
  ...
  refreshProjectSessions: (hostId: string, projectId: string, limit = 50) => request<ProviderSession[]>(`/api/projects/${hostId}/${projectId}/sessions/refresh?limit=${limit}`, { method: 'POST', body: '{}' }),
  ...
  externalSessionInput: (session: ProviderSession, message: string, delivery: 'steer' | 'queue') => request<{ ok: boolean; delivery: string; queued?: ExternalQueuedInput; run?: Run }>(`/api/external-sessions/${session.hostId}/${session.pid}/input`, { method: 'POST', body: JSON.stringify({ message, delivery, session_id: session.nativeSessionId, project_id: session.projectId, title: session.title }) }),
  externalAgentInput: (hostId: string, projectId: string, pid: number, sessionId: string | null | undefined, message: string, delivery: 'steer' | 'queue') => request<{ ok: boolean; delivery: string; queued?: ExternalQueuedInput; run?: Run }>(`/api/external-sessions/${hostId}/${pid}/input`, { method: 'POST', body: JSON.stringify({ message, delivery, session_id: sessionId, project_id: projectId }) }),
  ...
  adoptExternalTmux: (session: ProviderSession) => request<{ ok: boolean; tmux_name: string; tmux_access_command: string }>(`/api/external-sessions/${session.hostId}/${session.pid}/tmux/adopt`, { method: 'POST', body: JSON.stringify({ project_id: session.projectId, session_id: session.nativeSessionId }) }),
  moveExternalToTmux: (session: ProviderSession) => request<{ ok: boolean; status: string }>(`/api/external-sessions/${session.hostId}/${session.pid}/tmux/move`, { method: 'POST', body: JSON.stringify({ project_id: session.projectId, session_id: session.nativeSessionId }) }),
