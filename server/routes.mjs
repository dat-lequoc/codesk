...
export function registerRoutes(app, { store, gateway, broadcast, stateCache, mappers, ownership }) {
  ...
  app.post('/api/runs/:hostId/:id/models', async (req, res) => { try { res.json(await gateway.request(req.params.hostId, `/v1/runs/${req.params.id}/models`, { method: 'POST', body: '{}', timeout: 30000 })) } catch (error) { res.status(400).json({ error: error.message }) } })
  // Driving a harness's own picker takes several screen captures, so this one
  // waits longer than the default request timeout.
  app.post('/api/runs/:hostId/:id/model', async (req, res) => { try { const applied = await gateway.request(req.params.hostId, `/v1/runs/${req.params.id}/model`, { method: 'POST', body: JSON.stringify(req.body || {}), timeout: 30000 }); scheduleHostRefresh(req.params.hostId, 0); res.json(applied) } catch (error) { res.status(400).json({ error: error.message }) } })
  ...
  app.post('/api/open-path', async (req,res)=>{try{const host=store.state.hosts.find((item)=>item.id===req.body.hostId);if(!host)throw new Error('Host not found');if(host.type!=='local')throw new Error('Opening remote folders requires an SSH-aware editor integration; the path remains available in Environment.');const opener=process.platform==='darwin'?'open':process.platform==='win32'?'explorer':'xdg-open';try{spawn(opener,[req.body.path],{detached:true,stdio:'ignore'}).unref()}catch{}res.json({ok:true})}catch(error){res.status(400).json({error:error.message})}})
